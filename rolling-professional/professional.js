/* professional.js — v6e (open checkout in new tab / anti-iframe)
   Changes vs v6d:
   - startPayment(): opens a blank tab immediately on user click, then navigates it to session.url
   - Fallback: if popup blocked, break out of iframe (window.top) or use current window
*/
...
function renderQuoteView() {
  ...
  if (btnPay) btnPay.addEventListener("click", (e) => { e.preventDefault(); startPayment(e); });
...
// *** NEW: open in new tab (or break out of iframe) ***
async function startPayment(clickEvent) {
  const words = sumWords();
  ...
  // Open a blank tab immediately (user gesture) to avoid popup blockers
  let checkoutWin = null;
  try {
    checkoutWin = window.open('about:blank', '_blank', 'noopener,noreferrer');
  } catch { /* some browsers may still block; we'll fallback */ }
  ...
  try {
    ...
    // Prefer the pre-opened tab
    if (checkoutWin && !checkoutWin.closed) {
      checkoutWin.location = url;
      checkoutWin.focus?.();
    } else {
      // Fallbacks: break out of iframe if needed, then navigate
    ...
    // Close the tab if something failed
